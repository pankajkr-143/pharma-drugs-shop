...
def open_win(): #OPENS MAIN MENU----------------------------------------------------------------------------MAIN MENU
    ...
    flag='apt'
    apt=Tk()
    apt.title("Pharma Details")
    Label(apt, text="Drugs & Pharma").grid(row=0,column=0,columnspan=3)
...
def make_bill(): # makes bill
    ...
    m+="  TITE Bhopal, Madhya Pradesh, T.M.\n\n"
    m+="-----------------------------------------------\n"
    if t==1:
        m+="Name: %s\n" % named
        m+="Address: %s\n" % addd
...
def again():    #for login window-----------------------------------------------------------------------------LOGIN WINDOW
    ...
    if flag=='apt':
        apt.destroy()
    root=Tk()
    root.title('Mackys Tech Pharmetic')
    Label(root,text='Pharma AND Drug Centre').grid(row=0,column=0,columnspan=5)
    Label(root,text="TITE, BHOPAL, MP, TM").grid(row=1,column=0,columnspan=5)
